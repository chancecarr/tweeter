...
export class Status {
  ...
  public static fromDto(dto: any | null): Status | null {
    if (dto == null) return null;

    const user = User.fromDto(dto.user ?? dto._user);
    const post = dto.post ?? dto._post;
    const timestamp = dto.timestamp ?? dto._timestamp;

    if (!user || post === undefined || timestamp === undefined) return null;
