  ...
  try {
    const userParams = {
      ...
      Item: {
        ...
        alias: "@Chance",
        imageUrl:
          "https://chance-340.s3.us-east-1.amazonaws.com/image/@test1.jpg",
        followeeCount: 0,
        followerCount: 10000,
      },
    };
    ...
    await client.send(new PutCommand(userParams));
    await client.send(new PutCommand(loginParams));
  } catch (error) {
    console.error("Error creating super user:", error);
  }
}

async function batchWriteUsers(n: number) {
  let userBatch = [];
  let loginBatch = [];
  let followBatch = [];
  for (let i = 1; i <= n; i++) {
    ...
    if (i % 8 === 0 || i === n) {
      ...
      if (i % 800 === 0) console.log(`...processed ${i} users`);
      userBatch = [];
      loginBatch = [];
      followBatch = [];
    }
...
async function sendBatch(users: any[], logins: any[], follows: any[]) {
  const params = {
    RequestItems: {
      users: users,
      logins: logins,
